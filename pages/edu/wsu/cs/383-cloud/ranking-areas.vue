...
ranked_crimes = crimes.copy()
        ...
        <p>
          Now we will decide how big a bin should be (in latitude / longitude values). We decided to use 0.005 as our variable. These bins are not too bug but not so small that they are practically single points like our original dataset.
        </p>
...
# groups = ranked_crimes.groupby(("latbin", "lonbin"))
        ...
        <p>
          In order to apply a weight to each type fo crime we need to look at all the types of incidents and categorize them. We did apply a some what arbitrary weight number to each group based on how they sounded. This is another possible source for error in determining how dangerous an area may be. The code clearly shows the numbers applied.
        </p>
...
['23G' 'Theft of Motor Vehicle Parts' 'ZZZ' 'Non-NIBRS Offense' '23H'
 ...
 'Curfew/Loitering/Vagrancy' '26D' 'Welfare Fraud']
        ...
        <p>
          I manually wrote a csv string based off of the unique incident types and added a weight value that represents higher level group of incident types. This will simplify the process of visualizing as well as automatically create our groups of incidents based on weight.
        </p>
        <pre>
<p>
# Key                       Value
...
ranked_crimes = pd.merge(ranked_crimes, weighted_incidents[["INCIDENT_CODE","INCIDENT_WEIGHT"]], left_on="IBR_TYPE", right_on="INCIDENT_CODE", how="left")
        ...
        <p>
          We are almost ready to create our new ranked data sets. We need to merge the weighted incident list data frame with the original data, then we can run Aggregate functions on the data frame to get our ranked Bins.
        </p>
        <p>
        ...
        <p>
          Finally we save these data frames as csv files so that way we can use them in other jobs in the future as well as share them with you, the reader. These data sets are available on the <a href="https://github.com/CraftingGamerTom/wsu-computer-science/tree/master/CS-383_Cloud-Computing_2020-Spring" target="_blank">Github</a>.
        </p>
...
rankedBins_day.to_csv(dataPath + 'WorcesterData_BinRanked_Day.csv')
    ...
    <b-row id="toc-5">
      <b-col cols="12">
        ...
        <p>
          In the end we are left with three new data sets. Each row represents a normalized danger level for a specific chunk per year, month, or year (described by data set name)
        </p>
        <pre>
<p>
Dataframe shape:
