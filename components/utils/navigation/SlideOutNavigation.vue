...
<template>
    <div id="slide-out-nav" v-bind:class="{ 'nav-screen-left': openOnRight, 'nav-screen-right': !openOnRight }">
        <img v-if="openOnRight" class="activator" id="activator-right" @click="toggleMenu" :src="activator" alt="activator-img">
        <slide id="slide-right" v-if="openOnRight" v-show="currentlyOpen">
            <ul>
                <li v-for="item in navItems" :key="item.link"><a :href="item.link"><img :src="item.img"></a></li>
            </ul>
        </slide>

        <slide id="slide-left" v-if="!openOnRight" v-show="currentlyOpen">
            <ul>
                <li v-for="item in navItems" :key="item.link"><a :href="item.link"><img :src="item.img"></a></li>
            </ul>
        </slide>
        <img v-if="!openOnRight" class="activator" v-bind:class="{ 'activator-active':toggle }" id="activator-left" @click="toggleMenu" :src="activator" alt="activator-img">
...
export default {
    ...
    data() {
        return {
            openOnRight: (this.side === "right" ? false : true),
            toggle: false,
            currentlyOpen: false,
            tween1: {},
            tween2: {}
    ...
    methods: {
        async toggleMenu() {
            ...
            if (this.toggle ? this.tween2.play() : this.tween2.reverse())

            // no display after closing
