<template>
  <div>
    <section>
      <b-row style="margin-left:0;margin-right:0;">
        <b-col cols="12">
          <b-container id="index-career-intro" class="container--1">
            <h2 style="padding-top:25px;padding-bottom:0px;margin-bottom:2rem;">
              Transparency, Leadership, Impact-Centered
            </h2>
            <p>
              I built a <strong>resume-customizer</strong> because it represents
              who I am. I'm not afraid to air out my dirty laundry by showing
              off <strong>past projects</strong>.
            </p>
            <p>
              I desire to <strong>lead the industry</strong> to new heights
              through <strong>learning to communicate</strong> and understand
              solutions better, together.
            </p>
            <p>
              I realize I'm not the right fit for every company, and not every
              company has the values I seek -
              <strong>thanks for checking out my website</strong>, in any case.
            </p>

            <b-row>
              <b-col cols="8" offset="2" md="6" offset-md="3">
                <div style="margin-bottom:2rem;">
                  <p
                    style="    text-align: left;margin-bottom: 0;margin-top: 2rem;font-weight: 600;"
                  >
                    What position are you looking to fill?
                  </p>
                  <b-form-select
                    v-model="selected"
                    :options="options"
                    size="sm"
                    class="mt-3"
                    @change="onCategorySelected()"
                  />
                </div>
              </b-col>
            </b-row>
          </b-container>
        </b-col>
      </b-row>
    </section>
    <section id="index-career-resume">
      <b-row style="margin-left:0;margin-right:0;">
        <b-col cols="12" offset="0" md="6" offset-md="3">
          <b-container
            class="container--1 resume-content"
          >
            <h3 style="padding-top:15px;">
              Thomas Rokicki <small style="font-size:1rem;">(he/him)</small>
            </h3>
            <h5 style="float:left">
              Rutland, MA
            </h5>
            <h5 style="float:right">
              thomasrokicki.com
            </h5>
            <hr style="clear:both;">
            <h6 style="padding-top:15px;text-decoration: underline;">
              Experience
            </h6>
            <div v-for="job in jobs" :key="job.id">
              <b-row
                v-if="
                  (selected == null && sortedProjects == null) ||
                    (sortedProjects != null && sortedProjects.has(job.id))
                "
                class="resume-item"
              >
                <!-- only show job if relevant project -->
                <b-col cols="12">
                  <h5 style="float:left;font-weight:800;">
                    {{ job.employer }}
                  </h5>
                  <p style="float:right;">
                    {{ job.startDate | friendlyDate_Month_Year }} -
                    {{
                      job.endDate
                        | ifDateNullSetPresent
                        | friendlyDate_Month_Year
                    }}
                  </p>

                  <p style="clear:both;">
                    <strong>{{ job.position }}</strong>
                  </p>

                  <p
                    class="resume-item-description"
                    style="margin-bottom:10px;"
                  >
                    {{ job.roleDescription }}
                  </p>

                  <!-- get stored project from main list or sorted list -->
                  <div v-if="selected == null && sortedProjects == null">
                    <div v-for="project in job.projects" :key="project.id">
                      <div
                        v-if="project.resumeDescription !== null"
                        class="resume-item-markdown"
                        v-html="
                          $md.render(project.resumeDescription.toString())
                        "
                      />
                    </div>
                  </div>
                  <div v-else>
                    <div
                      v-for="project in sortedProjects.get(job.id)"
                      :key="project.id"
                    >
                      <div
                        v-if="project.resumeDescription !== null"
                        class="resume-item-markdown"
                        v-html="
                          $md.render(project.resumeDescription.toString())
                        "
                      />
                    </div>
                  </div>
                </b-col>
              </b-row>
            </div>

            <h6 style="padding-top:15px;text-decoration: underline;">
              Education
            </h6>
            <div v-for="educationItem in educations" :key="educationItem.id">
              <b-row
                v-if="
                  (selected == null && sortedEducation == null) ||
                    (sortedEducation != null &&
                      sortedEducation.includes(educationItem.id))
                "
                class="resume-item"
              >
                <b-col cols="12">
                  <h5 style="float:left;font-weight:800;">
                    {{ educationItem.university }}
                  </h5>
                  <p style="float:right;">
                    {{ educationItem.startDate | friendlyDate_Year }} -
                    {{
                      educationItem.endDate
                        | ifDateNullSetPresent
                        | friendlyDate_Year
                    }}
                  </p>

                  <p style="clear:both;">
                    <strong>{{ educationItem.degree }},
                      {{ educationItem.major }}</strong>
                    {{ educationItem.gpa }} GPA
                  </p>

                  <p class="resume-item-description">
                    {{ educationItem.honors }}, {{ educationItem.groups }}
                  </p>
                </b-col>
              </b-row>
            </div>

            <h6 style="padding-top:15px;text-decoration: underline;">
              Skills
            </h6>

            <div style="display:inline-flex;">
              <div style="width:100%;margin-bottom:25px">
                <img v-for="skill in sortedSkills" :key="skill.id" :src="skill.avatar.url | contentSiteImageUrl" :title="skill.name" class="resume-skill-icon">
              </div>
            </div>

            <!-- <div v-if="recentProjects.fetchError" class="text--error">
              <h3>Error loading recent projects</h3>
              <p>tttt</p>
            </div>
            <b-row v-else>
              <b-col v-for="project in recentProjects.projects" :key="project.id" cols="12" md="6" lg="3">
                <b-container class="container--2">
                  <h1>t</h1>
                </b-container>
              </b-col>
            </b-row> -->

            <b-row>
              <b-col cols="8" offset="2" />
            </b-row>

            <b-row>
              <b-col style="text-align:center;" cols="12">
                <b-button
                  style="margin-bottom:15px;"
                  :to="'/contact'"
                >
                  Contact Me
                </b-button>
              </b-col>
            </b-row>
          </b-container>
        </b-col>
      </b-row>
    </section>
  </div>
</template>

<script>
import ENV from '@/env'

export default {
  filters: {
    ifDateNullSetPresent (date) {
      if (date) {
        return date
      } else {
        return 'Present'
      }
    },
    friendlyDate_Year (date) {
      if (date === 'Present') { return date }
      const friendlyDate = new Date(date)
      return friendlyDate.getFullYear()
    },
    friendlyDate_Month_Year (date) {
      const month = [
        'Jan',
        'Feb',
        'Mar',
        'Apr',
        'May',
        'Jun',
        'Jul',
        'Aug',
        'Sep',
        'Oct',
        'Nov',
        'Dec'
      ]

      if (date === 'Present') { return date }
      const friendlyDate = new Date(date)
      return (
        month[friendlyDate.getUTCMonth()] + ' ' + friendlyDate.getFullYear()
      )
    },
    contentSiteImageUrl (url) {
      return ENV.strapi.domain + url
    }
  },
  data () {
    return {
      selected: null,
      options: [
        { value: null, text: 'CV (All Experience)' },
        { value: 'fullstack', text: 'Full Stack Engineer' },
        { value: 'frontend', text: 'Web Frontend Developer' },
        { value: 'mobile', text: 'Mobile Developer' },
        { value: 'backend', text: 'Backend Engineer' },
        { value: 'software-developer', text: 'Software Developer' },
        { value: 'test-engineer', text: 'Test Engineer' },
        { value: 'entry-level', text: 'Entry Level' }
      ],
      jobs: [],
      educations: [],
      sortedProjects: null,
      sortedEducation: null,
      sortedSkills: []
    }
  },
  async mounted () {
    try {
      this.jobs = await this.$strapi.$jobs.find({ _sort: 'endDate:DESC' })
      this.educations = await this.$strapi.$educations.find({
        _sort: 'endDate:DESC'
      })
      // this.sortedSkills = this.sortSkills(this.jobs)
      this.sortedSkills = await this.$strapi.$tags.find()
    } catch (error) {
      console.log(error)
    }
  },
  methods: {
    sortSkills (jobs) {
      let localProjects = null
      if (this.sortedProjects == null) {
        const projects = []
        for (const job in this.jobs) {
          for (const project in job.projects) {
            projects.push(project)
          }
        }
        localProjects = projects
      } else {
        localProjects = this.sortedProjects.values()
      }

      console.log(this.jobs)
      console.log(this.sortedProjects)
      console.log(localProjects)
      const skillsMap = new Map()
      for (let i = 0; i < localProjects.length; i++) {
        const project = localProjects[i]

        for (const tag in project.tags) { // skills
          if (skillsMap.has(tag.id)) {
            // do nothing, skill exists
          } else {
            skillsMap.set(tag.id, tag)
          }
        }
      }
      console.log(skillsMap)
      return skillsMap
    },
    async onCategorySelected () {
      if (this.selected) {
        const response = await this.$strapi.$categories.find({
          slug: this.selected
        })
        const category = response[0]

        // filter projects
        const projectsMap = new Map()
        const skillsMap = new Map()
        console.log(skillsMap)
        for (let i = 0; i < category.projects.length; i++) {
          const project = category.projects[i]
          const jobId = project.job
          if (projectsMap.has(jobId)) { // projects
            const sorted = projectsMap.get(jobId)
            sorted.push(project)
            projectsMap.set(jobId, sorted)
          } else {
            projectsMap.set(jobId, [project])
          }
        }
        this.sortedProjects = projectsMap

        // filter skills
        this.sortedSkills = this.sortSkills(projectsMap.values())

        // filter educations
        const educationList = []
        for (let i = 0; i < category.educations.length; i++) {
          educationList.push(category.educations[i].id)
        }
        this.sortedEducation = educationList
      } else {
        this.sortedProjects = null
        this.skillsMap = null
        this.sortedEducation = null
      }
    }
  }
}
</script>

<style lang="scss" scoped>
#index-career-resume {
  user-select: none;

  & .resume-content {
    // box-shadow: none;
    // border-radius: 1px;
    // background-color: aliceblue;
    text-align: left;
  }
  & .resume-item {
    padding-bottom: 20px;
    & h6,
    p {
      padding-top: 0;
      margin-bottom: 0;
    }
    & .resume-item-description {
      line-height: 1.1em;
      font-size: 0.9em;
      padding-top: 2px;
    }
    & .resume-item-markdown::v-deep {
      font-style: italic;
      font-weight: 300;
      line-height: 1em;
      font-size: 0.9em;
      & ul {
        list-style-type: square;
        margin: 0;
        padding-left: 20px;
      }
      & li {
        margin: 5px 0;
      }
      & p {
        margin: 0;
      }
    }
  }
  & .resume-skill-icon {
    height:5vh;
    width:5vh;
    border-radius:5px;
    object-fit:cover;
    margin: 2px;
    &:hover {

    }
  }
}
</style>
